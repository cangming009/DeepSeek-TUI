    ...
    fn load_state(&mut self) -> Result<()> {
        ...
        for persisted in state.agents {
            ...
            if matches!(status, SubAgentStatus::Running) {
                status = SubAgentStatus::Failed(SUBAGENT_RESTART_REASON.to_string());
            }
...
mod tests {
    ...
    #[test]
    fn test_persist_and_reload_marks_running_agent_as_failed() {
        let tmp = tempdir().expect("tempdir");
        ...
        assert!(matches!(
            snapshot.status,
            SubAgentStatus::Failed(ref message) if message.contains(SUBAGENT_RESTART_REASON)
        ));
    }
}
